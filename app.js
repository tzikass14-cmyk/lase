...
document.querySelectorAll('a[href^="#"]').forEach(link => {
  if (link.closest('.float-nav')) return;
...
(function () {
  ...
  function cacheOffsets() {
    ...
    items.forEach(item => {
      ...
      if (href && href.startsWith('#')) {
        ...
        sectionOffsets.push(el ? el.offsetTop - NAV_HEIGHT : Infinity);
      } else {
        sectionOffsets.push(Infinity);
      }
    });
  }
  ...
  function updateActive() {
    ...
    for (let i = 0; i < items.length; i++) {
      if (sectionOffsets[i] <= scrollY) current = items[i];
    }
    moveLamp(current);
  }
  ...
  window.addEventListener('resize', () => { cacheOffsets(); updateActive(); });
  ...
  items.forEach(item => {
    item.addEventListener('click', e => {
      ...
      moveLamp(item);
      clickLock = true;
      window.scrollTo({ top: target.offsetTop - NAV_HEIGHT, behavior: 'instant' });
...
document.querySelectorAll('.ui-img-wrap').forEach(wrap => {
  wrap.addEventListener('click', () => {
    const el = wrap.querySelector('iframe') || wrap.querySelector('video');
...
function applyCoupon() {
  ...
  if (!discount) {
    ...
    document.getElementById('discPv').style.display = 'none';
    return;
  }

  const price3m = (60 * (1 - discount / 100)).toFixed(2);
  const priceLt = (90 * (1 - discount / 100)).toFixed(2);
  const pricePv2m = (55 * (1 - discount / 100)).toFixed(2);
  const pricePv3m = (80 * (1 - discount / 100)).toFixed(2);
  const pricePvLt = (110 * (1 - discount / 100)).toFixed(2);
  ...
  document.getElementById('discPv').style.display = 'block';
  document.getElementById('discPv').textContent = `→ €${pricePv2m} / €${pricePv3m} / €${pricePvLt} with ${discount}% off`;
...
document.querySelectorAll('.price-card').forEach(card => {
  const tiers = card.querySelectorAll('.private-tier');
  ...
  tiers.forEach(tier => {
    tier.addEventListener('click', () => {
      ...
      const patchEl = card.querySelector('li:nth-child(5)');

      if (descEl) descEl.textContent = tier.dataset.desc;
      if (btnEl) btnEl.textContent = tier.dataset.btn;
      if (noteEl) noteEl.textContent = tier.dataset.note;

      if (card.dataset.card === 'elite' && patchEl) {
        patchEl.textContent = tier.dataset.tier === 'elt' ? 'Priority same-day patches' : 'All patches during plan';
      }
    });
  });
});
...
(function () {
  ...
  const plans = {
    '3m': { name: '3 Months Access', price: '€60' },
    'lt': { name: 'Lifetime Access', price: '€90' },
    'pv-2m': { name: 'Private · 2 Month', price: '€55' },
    'pv-3m': { name: 'Private · 3 Month', price: '€80' },
    'pv-lt': { name: 'Private · Lifetime', price: '€110' },
  };
...
function runTerminal(plan) {
  ...
  const planLabel = plan === 'lt' ? 'lifetime' : plan === 'pv-lt' ? 'private-lifetime' : plan === 'pv-3m' ? 'private-3m' : plan === 'pv-2m' ? 'private-2m' : '6-month';
  const lines = [
